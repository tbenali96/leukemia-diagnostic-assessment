...
def make_predictions(data_for_prediction: pd.DataFrame, model_path: str, scaler_path: str, pca_path:str) -> np.array:
    logging.info("Building the features for the test dataset")
    data = build_features(raw_data=data_for_prediction, train_or_test="test", scaler_path=scaler_path, pca_path=pca_path)
